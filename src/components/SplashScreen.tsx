...
export const CustomSplashScreen: React.FC<CustomSplashScreenProps> = ({ onFinish }) => {
  ...
  const progressAnim = useRef(new Animated.Value(0)).current; // Для прогресс-бара
  ...
  useEffect(() => {
    ...
    // Анимация прогресс-бара (не использует useNativeDriver)
    Animated.timing(progressAnim, {
      toValue: 1,
      duration: 2500,
      useNativeDriver: false, // Важно: false для width
    }).start();
